...
def load_synthetic_data() -> dict[str, Any]:
    """Load Korean synthetic benchmark data."""
    filepath = Path(__file__).parent / "data" / "ko_synthetic.json"
    if not filepath.exists():
        raise FileNotFoundError(f"Korean synthetic data not found: {filepath}")
    ...
    for sentence in sentences:
        ...
        for token in tokens:
            if token.phonemes:
                ...
                # Track unique phonemes (character-based for Korean)
...
def print_results(results: list[ConfigBenchmark], verbose: bool = False):
    ...
    print("\n" + "=" * 80)
    print("Korean G2P Configuration Comparison")
    print("=" * 80)
    print(
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark Korean G2P configurations")
    ...
    # Load data
    print("Loading Korean synthetic data...")

#!/usr/bin/env python3
"""Comprehensive benchmark comparison for English (US) G2P configurations.

This script tests all possible configurations of kokorog2p for US English:
...
Usage:
    python benchmark_en_us_comparison.py
    python benchmark_en_us_comparison.py --output results.json
    python benchmark_en_us_comparison.py --verbose
"""
...
def print_detailed_results(result: ConfigBenchmark, show_errors: bool = True) -> None:
    ...
    if show_errors and result.errors:
        print(
            f"\nSample errors (showing {min(5, len(result.errors))} of {len(result.errors)}):"
        )
...
def main():
    ...
    parser.add_argument(
        ...
        "-l",
        default="en-us",
        help="Language to benchmark (default: en-us)",
    )
    parser.add_argument(
        "--output",
        "-o",
    ...
    print(
        f"Loaded {len(data['sentences'])} sentences from {data['metadata']['description']}"
    )
    ...
    if results:
        ...
        print(
            f"\nBest accuracy:  {best_accuracy.config_name} ({best_accuracy.accuracy_percent:.1f}%)"
        )
        print(
            f"Fastest:        {best_speed.config_name} ({best_speed.words_per_second:,.0f} words/sec)"
        )

...
class ToneSandhi:
    """Mandarin tone sandhi processor."""
    ...
    def _split_word(self, word: str) -> list[str]:
        word_list = list(jieba.cut_for_search(word))
    ...
    def _merge_yi(self, seg: list[tuple[str, str]]) -> list[tuple[str, str]]:
        ...
        for i, (word, pos) in enumerate(seg):
            ...
            if (
                ...
                and seg[i - 1][1] == "v"
                and seg[i + 1][1] not in X_ENG
            ):
                new_seg[-1] = (
                    new_seg[-1][0] + YI + seg[i + 1][0],
                    new_seg[-1][1],
                )
                skip_next = True
            else:
    ...
    def _merge_continuous_three_tones(
        ...
        for i, (word, pos) in enumerate(seg):
            if (
                ...
                # if the last word is reduplication, not merge
                if (
    ...
    def pre_merge_for_modify(self, seg: list[tuple[str, str]]) -> list[tuple[str, str]]:
        """Pre-merge segments for tone modification."""
    ...
    def modified_tone(self, word: str, pos: str, finals: list[str]) -> list[str]:
        """Apply tone sandhi rules to finals."""

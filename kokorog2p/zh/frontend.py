"""Chinese frontend for G2P processing.
...
# Add punctuation and tone numbers to the map
for p in ';:,.!?/—…"()"" 12345R':
    if p not in ZH_MAP:
        ZH_MAP[p] = p
...
class ZHFrontend:
    """Chinese frontend for phoneme generation using Zhuyin-based representation."""

    def __init__(self, unk: str = "") -> None:
        """Initialize the Chinese frontend.
    ...
    def _get_initials_finals(self, word: str) -> tuple[list[str], list[str]]:
        ...
        # after pypinyin==0.44.0, '嗯' need to be n2
    ...
    def _merge_erhua(
        ...
        # 发音
        if word not in self.must_erhua and (
            word in self.not_erhua or pos in {"a", "j", "nr"}
        ):
            return initials, finals
    ...
    def __call__(
        ...
        Args:
            ...
            with_erhua: Whether to apply erhua processing.
        ...
        for word, pos in seg_cut:
            if pos == "x" and "\u4e00" <= min(word) and max(word) <= "\u9fff":
                ...
                pos = "x"
            tk = GToken(text=word, tag=pos, whitespace="")
            if pos in ("x", "eng"):
                if not word.isspace():
                ...
                tokens
                and tokens[-1].tag not in ("x", "eng")
                and not tokens[-1].whitespace
            ):
                tokens[-1].whitespace = "/"
